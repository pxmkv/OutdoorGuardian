...
def complementary_filter(pitch, roll, yaw, gyro_data, dt, alpha=0.98):
    ...
    yaw = alpha * yaw + (1 - alpha) * math.atan2(math.sqrt(gyro_data[1] * gyro_data[1] + gyro_data[0] * gyro_data[0]), gyro_data[2]) * 180 / math.pi
...
while True:
    ...
    pitch, roll, yaw = complementary_filter(pitch, roll, yaw, gyro_data, dt)
    ...
    display.text('p '+str(int(pitch)), 0, 10, 1)
    display.text('r '+str(int(roll)), 0, 20, 1)
    display.text('y '+str(int(yaw)), 0, 30, 1)
    display.text('a '+str((int(accel_data[0]), int(accel_data[1]), int(accel_data[2]))), 0, 40, 1)
    display.text('g '+str((int(gyro_data[0]), int(gyro_data[1]), int(gyro_data[2]))), 0, 50, 1)
